...
void PairSPHRhoSumMultiphase::settings(int narg, char **arg) {
  if (narg != 1)
    error->all(FLERR,
        "Illegal number of setting arguments for pair_style sph/rhosum/multiphase");
...
void PairSPHRhoSumMultiphase::coeff(int narg, char **arg) {
  if (narg != 3)
    error->all(FLERR,"Incorrect number of args for sph/rhosum/multiphase coefficients");
  if (!allocated)
    allocate();
...
double PairSPHRhoSumMultiphase::init_one(int i, int j) {
  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/rhosum/multiphase coeffs are not set");
  }
