...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     MESO_E,MESO_CV,MESO_RHO,MESO_VISCOSITY,MESO_RADIUS,MESO_CSOUND,MESO_PHI,INAME,DNAME};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    } else if (strcmp(arg[iarg],"meso_viscosity") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->viscosity_flag_)
        error->all(FLERR,"Cannot set meso_viscosity for this atom style");
      set(MESO_VISCOSITY);
      iarg += 2;

    } else if (strcmp(arg[iarg],"meso_radius") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->radius_flag)
        error->all(FLERR,"Cannot set meso_radius for this atom style");
      set(MESO_RADIUS);
      iarg += 2;

    } else if (strcmp(arg[iarg],"meso_csound") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->csound_flag)
        error->all(FLERR,"Cannot set meso_csound for this atom style");
      set(MESO_CSOUND);
      iarg += 2;

    } else if (strcmp(arg[iarg],"meso_phi") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->phi_flag)
        error->all(FLERR,"Cannot set meso_phi for this atom style");
      set(MESO_PHI);
      iarg += 2;
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == MESO_VISCOSITY) atom->viscosity_[i] = dvalue;
    else if (keyword == MESO_RADIUS) atom->radius[i] = dvalue;
    else if (keyword == MESO_CSOUND) atom->csound[i] = dvalue;
    else if (keyword == MESO_RHO) atom->rho[i] = dvalue;
    else if (keyword == MESO_PHI) atom->phi[i] = dvalue;
