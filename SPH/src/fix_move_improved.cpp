...
FixMoveImproved::FixMoveImproved(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 4) error->all(FLERR,"Illegal fix move/improved command");
  ...
  vxvarstrlinear = vyvarstrlinear = vzvarstrlinear = NULL;

  if (strcmp(arg[3],"linear") == 0) {
    if (narg < 7) error->all(FLERR,"Illegal fix move/improved command");
    iarg = 7;
    mstyle = LINEAR;
    if (strcmp(arg[4],"NULL") == 0) {vxvarstrlinear = NULL; vxflag = 0;}
    else if (strstr(arg[4],"v_") == arg[4]) {
      int n = strlen(&arg[4][2]) + 1;
      vxvarstrlinear = new char[n];
      strcpy(vxvarstrlinear,&arg[4][2]);
	  vxflag = 1;
    } else error->all(FLERR,"Illegal fix move/improved command");
    if (strcmp(arg[5],"NULL") == 0) {vyvarstrlinear = NULL; vyflag = 0;}
    else if (strstr(arg[5],"v_") == arg[5]) {
      int n = strlen(&arg[5][2]) + 1;
      vyvarstrlinear = new char[n];
      strcpy(vyvarstrlinear,&arg[5][2]);
	  vyflag = 1;
    } else error->all(FLERR,"Illegal fix move/improved command");
    if (strcmp(arg[6],"NULL") == 0) {vzvarstr = NULL; vzflag = 0;}
    else if (strstr(arg[6],"v_") == arg[6]) {
      int n = strlen(&arg[6][2]) + 1;
      vzvarstrlinear = new char[n];
      strcpy(vzvarstrlinear,&arg[6][2]);
	  vzflag = 1;
    } else error->all(FLERR,"Illegal fix move/improved command");

  } else if (strcmp(arg[3],"wiggle") == 0) {
    if (narg < 8) error->all(FLERR,"Illegal fix move/improved command");
    iarg = 8;
    mstyle = WIGGLE;
    ...
    if (narg < 11) error->all(FLERR,"Illegal fix move/improved command");
    iarg = 11;
    mstyle = ROTATE;
    ...
    if (narg < 10) error->all(FLERR,"Illegal fix move/improved command");
    iarg = 10;
    mstyle = VARIABLE;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"units") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix move/improved command");
  ...
  if (mstyle == ROTATE) {
    ...
    if (len == 0.0)
      error->all(FLERR,"Zero length rotation vector with fix move/improved");
    runit[0] = axis[0]/len;
    runit[1] = axis[1]/len;
    runit[2] = axis[2]/len;
  }
...
FixMoveImproved::~FixMoveImproved()
{
  ...
  delete [] vyvarstr;
  delete [] vzvarstr;
  delete [] vxvarstrlinear;
  delete [] vyvarstrlinear;
  delete [] vzvarstrlinear;
}

/* ---------------------------------------------------------------------- */

int FixMoveImproved::setmask()
{
  int mask = 0;
...
void FixMoveImproved::init()
{
  ...
  velocitylinearflag = 0;

  if (mstyle == LINEAR) {
    if (vxvarstrlinear) {
      vxvarlinear = input->variable->find(vxvarstrlinear);
      if (vxvarlinear < 0) error->all(FLERR,
                                "Variable name for fix move/improved does not exist");
      if (input->variable->equalstyle(vxvarlinear)) vxvarlinearstyle = EQUAL;
      else if (input->variable->atomstyle(vxvarlinear)) vxvarlinearstyle = ATOM;
      else error->all(FLERR,"Variable for fix move/improved is invalid style");
    }
    if (vyvarstrlinear) {
      vyvarlinear = input->variable->find(vyvarstrlinear);
      if (vyvarlinear < 0) error->all(FLERR,
                                "Variable name for fix move/improved does not exist");
      if (input->variable->equalstyle(vyvarlinear)) vyvarlinearstyle = EQUAL;
      else if (input->variable->atomstyle(vyvarlinear)) vyvarlinearstyle = ATOM;
      else error->all(FLERR,"Variable for fix move/improved is invalid style");
    }
    if (vzvarstrlinear) {
      vzvarlinear = input->variable->find(vzvarstrlinear);
      if (vzvarlinear < 0) error->all(FLERR,
                                "Variable name for fix move/improved does not exist");
      if (input->variable->equalstyle(vzvarlinear)) vzvarlinearstyle = EQUAL;
      else if (input->variable->atomstyle(vzvarlinear)) vzvarlinearstyle = ATOM;
      else error->all(FLERR,"Variable for fix move/improved is invalid style");
    }

    if (vxvarstrlinear && vxvarlinearstyle == ATOM) velocitylinearflag = 1;
    if (vyvarstrlinear && vyvarlinearstyle == ATOM) velocitylinearflag = 1;
    if (vzvarstrlinear && vzvarlinearstyle == ATOM) velocitylinearflag = 1;
  }
...
void FixMoveImproved::initial_integrate(int vflag)
{
  double dtfm;
  ...
  if (mstyle == LINEAR) {
    ...
    if (vxvarstrlinear) {
      if (vxvarlinearstyle == EQUAL) vx = input->variable->compute_equal(vxvarlinear);
      else input->variable->compute_atom(vxvarlinear,igroup,&velocity[0][0],3,0);
    }
    if (vyvarstrlinear) {
      if (vyvarlinearstyle == EQUAL) vy = input->variable->compute_equal(vyvarlinear);
      else input->variable->compute_atom(vyvarlinear,igroup,&velocity[0][1],3,0);
    }
    if (vzvarstrlinear) {
      if (vzvarlinearstyle == EQUAL) vz = input->variable->compute_equal(vzvarlinear);
      else input->variable->compute_atom(vzvarlinear,igroup,&velocity[0][2],3,0);
    }
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (vxflag) {
          if (vxvarlinearstyle == EQUAL) v[i][0] = vx;
        ...
        if (vyflag) {
          if (vyvarlinearstyle == EQUAL) v[i][1] = vy;
        ...
        if (vzflag) {
          if (vzvarlinearstyle == EQUAL) v[i][2] = vz;
...
void FixMoveImproved::final_integrate()
{
  double dtfm;
...
void FixMoveImproved::initial_integrate_respa(int vflag, int ilevel, int iloop)
{
...
void FixMoveImproved::final_integrate_respa(int ilevel, int iloop)
{
...
double FixMoveImproved::memory_usage()
{
...
void FixMoveImproved::write_restart(FILE *fp)
{
  int n = 0;
  double list[1];
  list[n++] = time_origin;
...
void FixMoveImproved::restart(char *buf)
{
  int n = 0;
...
void FixMoveImproved::grow_arrays(int nmax)
{
...
void FixMoveImproved::copy_arrays(int i, int j, int delflag)
{
...
void FixMoveImproved::set_arrays(int i)
{
  double **x = atom->x;
  imageint *image = atom->image;
  int *mask = atom->mask;
  ...
  if (mstyle == VARIABLE)
    error->all(FLERR,"Cannot add atoms to fix move/improved variable");
...
int FixMoveImproved::pack_exchange(int i, double *buf)
{
  buf[0] = xoriginal[i][0];
  buf[1] = xoriginal[i][1];
  buf[2] = xoriginal[i][2];
...
int FixMoveImproved::unpack_exchange(int nlocal, double *buf)
{
...
int FixMoveImproved::pack_restart(int i, double *buf)
{
  buf[0] = 4;
  buf[1] = xoriginal[i][0];
  buf[2] = xoriginal[i][1];
...
void FixMoveImproved::unpack_restart(int nlocal, int nth)
{
...
int FixMoveImproved::maxsize_restart()
{
  return 4;
}
...
int FixMoveImproved::size_restart(int nlocal)
{
  return 4;
}
...
void FixMoveImproved::reset_dt()
{
  error->all(FLERR,"Resetting timestep size is not allowed with fix move/improved");
}
