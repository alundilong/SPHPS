...
Atom::Atom(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  phi_flag = csound_flag = 0;

  colorgradient_flag = 0;
  epsilon_flag_ = artStress_flag_ = sigma_flag_ = epsilonBar_flag_ = R_flag_ = tau_flag_ = p_flag_ = viscosity_flag_ = dTau_flag_ = dSigma_flag_ = 0;
  xOld_flag_ = vOld_flag_ = rhoOld_flag_ = eOld_flag_ = tauOld_flag_ = phiOld_flag_ = 0; 
  // sigmaOld_flag_ = 0;
...
Atom::~Atom()
{
  ...
  memory->destroy(vest);
  memory->destroy(phi);
  memory->destroy(csound);
  memory->destroy(vXSPH_);
  memory->destroy(tau_);
  memory->destroy(dTau_);
  memory->destroy(dSigma_);
  memory->destroy(epsilon_);
  memory->destroy(epsilonBar_);
  memory->destroy(p_);
  memory->destroy(viscosity_);
  memory->destroy(R_);
  memory->destroy(sigma_);
  memory->destroy(artStress_);

  memory->destroy(xOld_);
  memory->destroy(vOld_);
  memory->destroy(rhoOld_);
  memory->destroy(phiOld_);
  memory->destroy(eOld_);
  memory->destroy(tauOld_);
//  memory->destroy(sigmaOld_);
...
void *Atom::extract(char *name)
{
  ...
  if (strcmp(name,"dphi") == 0) return (void *) dphi;
  if (strcmp(name,"e") == 0) return (void *) e;
  if (strcmp(name,"de") == 0) return (void *) de;
  if (strcmp(name,"cv") == 0) return (void *) cv;
  if (strcmp(name,"vest") == 0) return (void *) vest;
  if (strcmp(name,"vXSPH") == 0) return (void *) vXSPH_;
