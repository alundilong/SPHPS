...
PairSPHViscousForce::PairSPHViscousForce(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
}
...
PairSPHViscousForce::~PairSPHViscousForce() {
  if (allocated) {
...
void PairSPHViscousForce::compute(int eflag, int vflag) {
...
void PairSPHViscousForce::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHViscousForce::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/viscousforce");
}
...
void PairSPHViscousForce::coeff(int narg, char **arg) {
  if (narg != 3)
    error->all(FLERR,"Incorrect number of args for pair_style sph/viscousforce coefficients");
  if (!allocated)
    allocate();
...
double PairSPHViscousForce::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/viscousforce coeffs are not set");
  }
...
double PairSPHViscousForce::single(int i, int j, int itype, int jtype,
