...
#include "stdio.h"
#include "string.h"
#include "fix_meso_PEC_solidMech.h"
#include "math.h"
#include "stdlib.h"
#include "string.h"
#include "atom.h"
...
FixMesoPECSolidMech::FixMesoPECSolidMech(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg) {

  if ((atom->e_flag != 1) || (atom->rho_flag != 1))
    error->all(FLERR,
        "fix meso/PEC/solidMech command requires atom_style with both energy and density");

  if (narg != 3)
    error->all(FLERR,"Illegal number of arguments for fix meso/PEC/solidMech command");
...
FixMesoPECSolidMech::~FixMesoPECSolidMech()
{
}

/* ---------------------------------------------------------------------- */

int FixMesoPECSolidMech::setmask() {
  int mask = 0;
...
void FixMesoPECSolidMech::init() {
  dtv = update->dt;
  dtf = 0.5 * update->dt * force->ftm2v;
}

void FixMesoPECSolidMech::setup_pre_force(int vflag)
{
  // set vest equal to v 
  double **v = atom->v;
  double **x = atom->x;
...
void FixMesoPECSolidMech::initial_integrate(int vflag) {
...
void FixMesoPECSolidMech::final_integrate() {
...
void FixMesoPECSolidMech::reset_dt() {
  dtv = update->dt;
  dtf = 0.5 * update->dt * force->ftm2v;
