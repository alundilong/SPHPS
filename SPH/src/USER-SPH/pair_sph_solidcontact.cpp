...
PairSPHSolidContact::PairSPHSolidContact(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
}
...
PairSPHSolidContact::~PairSPHSolidContact() {
  if (allocated) {
...
void PairSPHSolidContact::init_style() {
...
void PairSPHSolidContact::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if (ndim==2) {
		...
		double lambda1 = rho[j]*csound[j]*rho[i]*csound[i]/(rho[j]*csound[j] + rho[i]*csound[i]);
		double lambda2 = Ei*Ej/(Ei+Ej)/d0;
...
void PairSPHSolidContact::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHSolidContact::settings(int narg, char **arg) {
  if (narg != 0)
    error->all(FLERR,
	       "Illegal number of setting arguments for pair_style sph/solidcontact");
}
...
void PairSPHSolidContact::coeff(int narg, char **arg) {
  if (narg != 3+5)
    error->all(FLERR,"Incorrect number of args for pair_style sph/solidcontact coefficients");
  if (!allocated)
    allocate();
...
double PairSPHSolidContact::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/solidcontact coeffs are not set");
  }
...
double PairSPHSolidContact::single(int i, int j, int itype, int jtype,
