...
#include "stdlib.h"
#include "string.h"
#include "fix_deposit_modified.h"
#include "atom.h"
#include "atom_vec.h"
#include "molecule.h"
#include "force.h"
...
FixDepositModified::FixDepositModified(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 7) error->all(FLERR,"Illegal fix deposit/modified command");
  ...
  if (seed <= 0) error->all(FLERR,"Illegal fix deposit/modified command");
  ...
  if (mode == ATOM && (ntype <= 0 || ntype > atom->ntypes))
    error->all(FLERR,"Invalid atom type in fix deposit/modified command");

  // error checks on region and its extent being inside simulation box

  if (iregion == -1) error->all(FLERR,"Must specify a region in fix deposit/modified");
  if (domain->regions[iregion]->bboxflag == 0)
    error->all(FLERR,"Fix deposit/modified region does not support a bounding box");
  if (domain->regions[iregion]->dynamic_check())
    error->all(FLERR,"Fix deposit/modified region cannot be dynamic");
  ...
  if (atom->tag_enable == 0)
    error->all(FLERR,"Cannot use fix_deposit/modified unless atoms have IDs");

  if (mode == MOLECULE) {
    for (int i = 0; i < nmol; i++) {
      if (onemols[i]->xflag == 0)
        error->all(FLERR,"Fix deposit/modified molecule must have coordinates");
      if (onemols[i]->typeflag == 0)
        error->all(FLERR,"Fix deposit/modified molecule must have atom types");
      if (ntype+onemols[i]->ntypes <= 0 || 
          ntype+onemols[i]->ntypes > atom->ntypes)
        error->all(FLERR,"Invalid atom type in fix deposit/modified mol command");
      
      if (atom->molecular == 2 && onemols != atom->avec->onemols)
        error->all(FLERR,"Fix deposit/modified molecule template ID must be same "
                   "as atom_style template ID");
      onemols[i]->check_attributes(0);

      // fix deposit/modified uses geoemetric center of molecule for insertion
  ...
  if (rigidflag && mode == ATOM)
    error->all(FLERR,"Cannot use fix deposit/modified rigid and not molecule");
  if (shakeflag && mode == ATOM)
    error->all(FLERR,"Cannot use fix deposit/modified shake and not molecule");
  if (rigidflag && shakeflag)
    error->all(FLERR,"Cannot use fix deposit/modified rigid and shake");
  ...
  memory->create(coords,natom_max,3,"deposit/modified:coords");
  memory->create(imageflags,natom_max,"deposit/modified:imageflags");
...
FixDepositModified::~FixDepositModified()
{
  delete random;
  delete [] molfrac;
  delete [] idrigid;
...
int FixDepositModified::setmask()
{
  int mask = 0;
  mask |= PRE_EXCHANGE;
  return mask;
...
void FixDepositModified::init()
{
  // set index and check validity of region

  iregion = domain->find_region(idregion);
  if (iregion == -1)
    error->all(FLERR,"Region ID for fix deposit/modified does not exist");
  ...
  if (rigidflag) {
    ...
    if (onemols != (Molecule **) fixrigid->extract("onemol",tmp))
      error->all(FLERR,
                 "Fix deposit/modified and fix rigid/small not using "
  ...
  if (shakeflag) {
    ...
    if (ifix < 0) error->all(FLERR,"Fix deposit/modified shake fix does not exist");
    fixshake = modify->fix[ifix];
    int tmp;
    if (onemols != (Molecule **) fixshake->extract("onemol",tmp))
      error->all(FLERR,"Fix deposit/modified and fix shake not using "
                 "same molecule template ID");
  }
}
...
void FixDepositModified::pre_exchange()
{
  ...
  milliseconds = (tv.tv_sec) + 0.000001 * tv.tv_usec;
  //printf("seconds = %d\n", seconds);
  random->reset(pow(milliseconds,1./3));
  ...
  if (!success && comm->me == 0)
    error->warning(FLERR,"Particle deposit/modifiedion was unsuccessful",0);
...
void FixDepositModified::find_maxid()
{
  tagint *tag = atom->tag;
...
void FixDepositModified::options(int narg, char **arg)
{
  // defaults
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix deposit/modified command");
      ...
      if (imol == -1)
        error->all(FLERR,"Molecule template ID for fix deposit/modified does not exist");
      mode = MOLECULE;
      ...
      if (mode != MOLECULE) error->all(FLERR,"Illegal fix deposit/modified command");
      if (iarg+nmol+1 > narg) error->all(FLERR,"Illegal fix deposit/modified command");
      ...
      if (molfrac[nmol-1] < 1.0-EPSILON || molfrac[nmol-1] > 1.0+EPSILON) 
        error->all(FLERR,"Illegal fix deposit/modified command");
      molfrac[nmol-1] = 1.0;
      iarg += nmol+1;
      ...
      else error->all(FLERR,"Illegal fix deposit/modified command");
      iarg += 2;
    } else if (strcmp(arg[iarg],"global") == 0) {
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix deposit/modified command");
      globalflag = 1;
      localflag = 0;
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix deposit/modified command");
      localflag = 1;
      globalflag = 0;
      lo = force->numeric(FLERR,arg[iarg+1]);
      hi = force->numeric(FLERR,arg[iarg+2]);
...
void FixDepositModified::write_restart(FILE *fp)
{
  int n = 0;
  double list[4];
  list[n++] = random->state();
...
void FixDepositModified::restart(char *buf)
{
  int n = 0;
...
void *FixDepositModified::extract(const char *str, int &itype)
{
