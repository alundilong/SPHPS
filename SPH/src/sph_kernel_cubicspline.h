...
namespace SPH_KERNEL_NS {
  ...
  inline double sph_kernel_cubicspline_der2(double s, double h, double hinv);
...
/* ----------------------------------------------------------------------
   2nd Derivative of cubic spline SPH kernel
   is equal to grad W if multiplied with radial unit vector
   h is kernel parameter
   s is distance normalized by h
   0.079577 is 1 over 4pi
...
inline double SPH_KERNEL_NS::sph_kernel_cubicspline_der2(double s,double h, double hinv)
{
    if (s < 1.)
    {
        return (0.079577*hinv*hinv*hinv*hinv*hinv * (6.*(2.-s) - 24.*(1.-s)));
    }
    else
    {
        return (0.079577*hinv*hinv*hinv*hinv*hinv * (6.*(2.-s)));
    }
}
