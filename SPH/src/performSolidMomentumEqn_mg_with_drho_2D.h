    ...
    for (jj = 0; jj < jnum; jj++) {
	  ...
	  if (rsq < cutsq[itype][jtype]) {
	    ...
	    wdeltap = SPH_KERNEL_NS::sph_kernel_cubicspline2d(deltap_/h, h, ih);
		...
		double eshear = 0.5*imass*jmass*(p_[i]/rhoi/rhoi + p_[j]/rhoj/rhoj + PIJ)*wfd*(delVdotDelR);
			  ...
			  eshear += -0.5*imass*jmass*wfd*fab*(tau_[i][0][0]*delvx*delx + tau_[i][0][1]*delvx*dely  \
							  + tau_[i][1][0]*delvy*delx + tau_[i][1][1]*delvy*dely  \
							  )/rhoi/rhoi;

		 	  eshear += -0.5*imass*jmass*fab*wfd*(tau_[j][0][0]*delvx*delx + tau_[j][0][1]*delvx*dely \
							  + tau_[j][1][0]*delvy*delx + tau_[j][1][1]*delvy*dely \
							  )/rhoj/rhoj;

			  eshear += -0.5*imass*jmass*wfd*fab*(artStress_[i][0][0]*delvx*delx + artStress_[i][0][1]*delvx*dely  \
							  + artStress_[i][1][0]*delvy*delx + artStress_[i][1][1]*delvy*dely \
							  );

		 	  eshear += -0.5*imass*jmass*fab*wfd*(artStress_[j][0][0]*delvx*delx + artStress_[j][0][1]*delvx*dely \
							  + artStress_[j][1][0]*delvy*delx + artStress_[j][1][1]*delvy*dely \
							  );
