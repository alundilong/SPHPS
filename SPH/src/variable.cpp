...
enum{DONE,ADD,SUBTRACT,MULTIPLY,DIVIDE,CARAT,MODULO,UNARY,
     ...
     RANDOM,NORMAL,CEIL,FLOOR,ROUND,RAMP,ZIGZAG,LINEARSTOP,MULTISTEPZIGZAG,STAGGER,LOGFREQ,STRIDE,
...
/* ----------------------------------------------------------------------
   ...
   customize by adding a function:
	 ...
	 zigzag(t0,A,T0), linearstop(t0,A,T0), multistepzigzag(t0,A,T0,N,TR)
---------------------------------------------------------------------- */
...
double Variable::collapse_tree(Tree *tree)
{
  ...
  if (tree->type == ZIGZAG) {
    ...
    if (tree->left->type != VALUE || tree->right->type != VALUE || tree->middle->type != VALUE) return 0.0;
    tree->type = VALUE;

	double t0 = arg1;
	double v0 = arg2/arg3*2.0;
	double T0 = arg3;

  	double timec = update->atime + (update->ntimestep-update->atimestep)*update->dt;
	double tmp = (timec - t0)/T0 - long((timec - t0)/T0);
	if(tmp < 0.5) tree->value = v0;
	else tree->value = -v0;
    return tree->value;
  }

  if (tree->type == LINEARSTOP) {
    arg1 = collapse_tree(tree->left);
    arg2 = collapse_tree(tree->middle);
    arg3 = collapse_tree(tree->right);
    if (tree->left->type != VALUE || tree->right->type != VALUE || tree->middle->type != VALUE) return 0.0;
	...
	double T0 = arg3;
  	...
  	double timec = update->atime + (update->ntimestep-update->atimestep)*update->dt;
	double tmp = timec - t0;
	if(tmp < T0) tree->value = v0;
	else tree->value = 0;
    return tree->value;
  }

  if (tree->type == MULTISTEPZIGZAG) {
    arg1 = collapse_tree(tree->left);
    arg2 = collapse_tree(tree->middle);
    arg3 = collapse_tree(tree->right);
    if (tree->left->type != VALUE || tree->right->type != VALUE || tree->middle->type != VALUE) return 0.0;
                            ...
                            double *argstack, int &nargstack)
{
  ...
  if (strcmp(word,"sqrt") && strcmp(word,"exp") &&
      ...
      strcmp(word,"zigzag") &&
      strcmp(word,"linearstop") &&
      strcmp(word,"multistepzigzag") &&
  ...
  } else if (strcmp(word,"zigzag") == 0) {
    if (narg != 3)
      error->all(FLERR,"Invalid math function in variable formula");
    if (update->whichflag == 0)
      error->all(FLERR,"Cannot use zigzag in variable formula between runs");
    if (tree) newtree->type = ZIGZAG;
    else {
  	  double timec = update->atime + (update->ntimestep-update->atimestep)*update->dt;
	  double t0 = value1;
	  double v0 = value2/value3*2.0;
	  double T0 = value3;
  ...
  } else if (strcmp(word,"linearstop") == 0) {
    if (narg != 3)
      error->all(FLERR,"Invalid math function in variable formula");
    if (update->whichflag == 0)
      error->all(FLERR,"Cannot use linearstop in variable formula between runs");
    if (tree) newtree->type = LINEARSTOP;
    else {
  	  double timec = update->atime + (update->ntimestep-update->atimestep)*update->dt;
	  double t0 = value1;
	  double v0 = value2/value3;
	  double T0 = value3;
  ...
  } else if (strcmp(word,"multistepzigzag") == 0) {
    if (narg != 3)
      error->all(FLERR,"Invalid math function in variable formula");
    if (update->whichflag == 0)
      error->all(FLERR,"Cannot use multistepzigzag in variable formula between runs");
    if (tree) newtree->type = MULTISTEPZIGZAG;
    else {
  	  double timec = update->atime + (update->ntimestep-update->atimestep)*update->dt;
	  double A = value1;
	  double T0 = value2;
	  ...
	  int n = 0;
	  double subT0 = T0/2./N;
	  double Tcri = 1.0 - TR/subT0;;

	  double value = 0.0;
	  double tmp = (timec - t0)/T0 - long((timec - t0)/T0);
	  double tmp2 = (timec - t0)/subT0 - long((timec - t0)/subT0);
	  if(tmp < 0.5){ 
		 if(tmp2 < Tcri)
			 value = v0;
		 else 
